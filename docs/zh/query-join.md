...
**需要注意的是，sqala的表连接方法不是大多数查询库采用的`from.join.on`形式，而是把`join`作为`from`的参数处理，这是符合SQL标准语义的，而且在子查询表、函数表、透视表等复杂情况下可以提供更统一的调用形式。**
...
各主流数据库最新版本实测支持程度如下（SQLServer有自己的`APPLY`语法，但无法跟标准SQL进行等价转换，所以sqala不做支持）：
