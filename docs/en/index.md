...
4. Capturing erroneous queries at compile time and returning semantic compile warnings:

    ![demo1](../../images/index-error.png)

5. Supporting multiple dialects including MySQL, PostgreSQL, and Oracle, with the same query expression generating different SQL by passing different dialect parameters.

6. Supporting Oracle's recursive query feature `CONNECT BY`, but generating standard SQL that is compatible across databases:
...
7. Beyond CRUD, sqala also supports advanced features like multidimensional grouping, subquery predicates, `LATERAL` subqueries, function tables, pivot tables, providing strong support for data analysis scenarios.

8. Using native SQL and automatically extracting result types from native SQL, accessing returned fields using `.`, maintaining type safety:
...
10. The `data` module provides JSON serialization, deserialization functions, and zero-overhead object mapping.

11. No additional dependencies beyond Scala and Java official libraries.
...
3. It is recommended to use the official Scala metals plugin with tools like VSCode, Vim, etc. The IDEA series currently cannot provide writing hints or correctly display the data types returned by queries.